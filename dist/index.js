...
import { HaloPSAClient } from './halopsa-client.js';
const config = {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        switch (name) {
            case 'halopsa_list_tables': {
                ...
                if (result?.report?.rows && Array.isArray(result.report.rows)) {
                    tables = result.report.rows.map((row) => row.Name || row.name);
            ...
            case 'halopsa_list_columns': {
                const { tableName, columnFilter } = args;
                if (!tableName) {
        ...
        `;
                ...
                if (result?.report?.rows && Array.isArray(result.report.rows)) {
                    result.report.rows.forEach((row) => {
        ...
        `;
            ...
            case 'halopsa_build_query': {
                const { tableName, columns, conditions, orderBy, limit } = args;
                if (!tableName) {
                    throw new Error('Table name is required');
                }
                let query = 'SELECT ';
            ...
            case 'halopsa_get_api_endpoint_details': {
                const { pathPattern, summaryOnly, includeSchemas, maxEndpoints, includeExamples } = args;
                if (!pathPattern) {
                    throw new Error('Path pattern is required');
                }
                result = await haloPSAClient.getApiEndpointDetails(pathPattern, summaryOnly, includeSchemas, maxEndpoints, includeExamples);
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify(result, null, 2)
            ...
            case 'halopsa_get_api_schemas': {
                const { schemaPattern, limit, skip, listNames } = args;
            ...
            case 'halopsa_api_call': {
                const { path, method, body, queryParams } = args;
                if (!path) {
